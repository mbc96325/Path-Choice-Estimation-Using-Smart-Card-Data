...
def process_afc_sjsc(df, flag):
    ...
    df['txn_dt'] = pd.to_datetime(df['txn_dt']).dt.time.astype(str)
...
class FilePreparation(object):
    ...
    def __init__(self, sim_time_period, file_path, file_octopus, file_network_operation,
                 file_time_table, output_file_path, Transfer_Walking_Time):
        ...
        self.route_choice_interval = CHOICE_INTERVAL
        ...
        self.txn_time_period = [self.carrier_time_period[0], self.carrier_time_period[1]]

        # reading and processing the external files
        self.Transfer_Walking_Time = pd.read_csv(Transfer_Walking_Time).fillna(0)  # single journey ticket data
    ...
    def create_tb_txn(self):
        ...
        self.tb_txn = tb_txn
        scale_demand = [self.tb_txn] * DEMAND_SCALE
        self.tb_txn = pd.concat(scale_demand)
        self.tb_txn['user_id'] = np.arange(len(self.tb_txn))
    ...
    def generate_path_attributes(self):
        ...
        od_path = \
            self.network_operation.loc[:, ['ORI_STN_NO', 'DES_STN_NO', 'HARBOUR_CO', 'PASS_LINE_NO', 'PASS_DIRECTION', 'PASS_STN', 'LINK_START',
                                          'LINK_END','TRAN_IND', 'DELAY_TIME']].drop_duplicates().reset_index(drop=True)
        ...
        # in-veh time
        temp = od_path.groupby(['ORI_STN_NO', 'DES_STN_NO', 'HARBOUR_CO'], sort=False).first()[['DELAY_TIME']].reset_index(drop=False)
        path_att = path_att.merge(temp, left_on=['ORI_STN_NO', 'DES_STN_NO', 'HARBOUR_CO'],
                                  right_on=['ORI_STN_NO', 'DES_STN_NO', 'HARBOUR_CO'])
        path_att = path_att.rename(columns = {'DELAY_TIME':'InVeh_time'})
        # num of transfer
        ...
        # map distance
        map_distance = pd.read_csv('External_data/Station_Map_Distances_clean.csv')
        distance_data = od_path.merge(map_distance[['link_start_id','link_end_id','Distance']],left_on = ['LINK_START','LINK_END'], right_on = ['link_start_id','link_end_id'], how='left')
        ...
        if len(distance_data_na)>0:
            print ('Following links have no distance data, filled by 1 cm')
            print (distance_data_na)
            distance_data = distance_data.fillna(1)

        distance_data = distance_data.groupby(['ORI_STN_NO', 'DES_STN_NO', 'HARBOUR_CO'], sort=False).sum().reset_index()[['ORI_STN_NO', 'DES_STN_NO', 'HARBOUR_CO','Distance']]
        # print(len(distance_data))
        path_att = path_att.merge(distance_data,left_on = ['ORI_STN_NO', 'DES_STN_NO', 'HARBOUR_CO'],right_on = ['ORI_STN_NO', 'DES_STN_NO', 'HARBOUR_CO'])
        # print(len(path_att))
        ...
        transfer_station = od_path.groupby(['ORI_STN_NO', 'DES_STN_NO', 'HARBOUR_CO','PASS_LINE_NO']).last().reset_index(drop=False)
        transfer_station = transfer_station.loc[transfer_station['DELAY_TIME']!=0].reset_index(drop=True)
        ...
        transfer_station['Line_after'] = transfer_station_after['PASS_LINE_NO']
        transfer_station['Dir_after'] = transfer_station_after['PASS_DIRECTION']
        transfer_station['Station_after'] = transfer_station_after['PASS_STN']
        transfer_station['platform_x'] = transfer_station['PASS_STN'].apply(str) + '_' + transfer_station['PASS_LINE_NO'].apply(str) + '_' + transfer_station['PASS_DIRECTION'].apply(str)
        transfer_station['platform_y'] = transfer_station['Station_after'].apply(str) + '_' + transfer_station['Line_after'].apply(str) + '_' + \
                                         transfer_station['Dir_after'].apply(str)
        print(len(transfer_station))
        transfer_station = transfer_station.merge(self.Transfer_Walking_Time,left_on = ['platform_x','platform_y'],right_on = ['platform_x','platform_y'],how = 'left')
        print(len(transfer_station))
        transfer_station = transfer_station.fillna(DEFAULT_TRANSFER_TIME)
        ...
        transfer_station = transfer_station.groupby(['ORI_STN_NO', 'DES_STN_NO', 'HARBOUR_CO']).sum().reset_index(drop=False)[
            ['ORI_STN_NO', 'DES_STN_NO', 'HARBOUR_CO', 'Transfer_time']]
        path_att = path_att.merge(transfer_station[['ORI_STN_NO', 'DES_STN_NO', 'HARBOUR_CO', 'Transfer_time']],
                                  left_on=['ORI_STN_NO', 'DES_STN_NO', 'HARBOUR_CO'], right_on = ['ORI_STN_NO', 'DES_STN_NO', 'HARBOUR_CO'],how = 'left').fillna(0)
        path_att['InVeh_time'] = path_att['InVeh_time'] - path_att['Transfer_time'] # subtract transfer time, update the inveh time
        ...
        path_att_time = path_att_time.merge(od_path_boarding, left_on = ['ORI_STN_NO', 'DES_STN_NO', 'HARBOUR_CO','TIME_INTERVAL'], right_on \
            = ['ORI_STN_NO', 'DES_STN_NO', 'HARBOUR_CO','TIME_INTERVAL'], how ='left').drop(columns = ['headway'])



        # expected denied boarding times-----
        first_boarding_station = od_path.groupby(['ORI_STN_NO','DES_STN_NO','HARBOUR_CO']).first().reset_index()
        transfer_station = od_path.loc[transfer_station_after_index,:]
        boarding_station = pd.concat([first_boarding_station,transfer_station],sort=False)
        boarding_station = boarding_station.sort_values(by=['ORI_STN_NO', 'DES_STN_NO', 'HARBOUR_CO', 'DELAY_TIME'], ascending=[True, True, True, False]).reset_index(drop=True)

        passenger_state = pd.read_csv('External_data/passenger_state_for_expected_DB_times.csv')
        passenger_state['arrival_time_interval'] = passenger_state['arrival_time'] // self.route_choice_interval * self.route_choice_interval
        station_metrix = passenger_state.groupby(['departure_station','departure_line','departure_direction',
                                           'arrival_time_interval']).mean()[['denied_boarding_times']].reset_index()
        station_metrix = station_metrix.rename(columns = {'departure_station':'PASS_STN','departure_line':'PASS_LINE_NO','departure_direction':'PASS_DIRECTION',
                                                          'arrival_time_interval':'TIME_INTERVAL'})
        denied_time = station_metrix.merge(boarding_station,left_on = ['PASS_STN','PASS_LINE_NO','PASS_DIRECTION'], right_on = ['PASS_STN','PASS_LINE_NO','PASS_DIRECTION'])
        denied_time = denied_time.groupby(['ORI_STN_NO', 'DES_STN_NO', 'HARBOUR_CO','TIME_INTERVAL']).sum()[['denied_boarding_times']].reset_index()
        # print (len(path_att_time))
        path_att_time = path_att_time.merge(denied_time[['ORI_STN_NO', 'DES_STN_NO', 'HARBOUR_CO','TIME_INTERVAL','denied_boarding_times']],how='left').fillna(0)
        # path_att_time_na = path_att_time.loc[path_att_time['denied_boarding_times'].isna()]
        # print(len(path_att_time))
        # ------******---
        ...
        path_att_time = path_att_time.rename(columns={'ORI_STN_NO': 'origin', 'DES_STN_NO': 'destination', 'HARBOUR_CO': 'path_id','TIME_INTERVAL':'time_interval',\
                                      ...
                                      'CF':'commonality_factor','denied_boarding_times':'DB_times'})
    ...
    def create_tb_itinerary(self):
        ...
        od_path = od_path.assign(new=od_path.PASS_LINE_NO.diff().ne(0).cumsum())
        temp = od_path.loc[:, ['ORI_STN_NO', 'DES_STN_NO', 'HARBOUR_CO', 'PASS_LINE_NO', 'PASS_DIRECTION','new']].drop_duplicates().reset_index(drop=True)
        temp['itinerary'] = temp.groupby(['ORI_STN_NO', 'DES_STN_NO', 'HARBOUR_CO']).cumcount()
        temp['itinerary'] += 1
        boarding_station = od_path.groupby(['ORI_STN_NO', 'DES_STN_NO', 'HARBOUR_CO', 'PASS_LINE_NO', 'PASS_DIRECTION','new'], sort=False).first()[['PASS_STN']].reset_index(drop=True)
        alighting_station = od_path.groupby(['ORI_STN_NO', 'DES_STN_NO', 'HARBOUR_CO', 'PASS_LINE_NO', 'PASS_DIRECTION','new'], sort=False).last()[['PASS_STN']].reset_index(drop=True)
        ...
        self.tb_itinerary = self.tb_itinerary.drop(columns = ['new'])
    ...
    def create_tb_carrier_network(self):
        ...
        (self.time_table['Dep_To_ID']>self.time_table['Arr_From_ID'])]
...
# -----------------------------------------  Main Function ---------------------------------------------------
def main(para,network_file,time_table_file,afc_file,sjsc_file, Transfer_Walking_Time, date, name):

    global TIME_TABLE_VAR, DATE, TEST
    para['time_table_var'] = para['time_table_var'][1:len(para['time_table_var'])-1]

    if len(para['time_table_var']) > 0:
        TIME_TABLE_VAR = [float(para['time_table_var'].split('_')[0]), float(para['time_table_var'].split('_')[1])]
    else:
        TIME_TABLE_VAR = [0, 0]

    # Variable settings
    date2 = para['date']
    DATE = date[1:len(date2) - 1]
    ...
    path_external_data = ''
    if TEST == 0:
        output_file_path =  'Assignment_' + str(time_start) + '-' + str(time_end) + '_' + date + name +  '/'
    else:
        output_file_path = 'Assignment_TEST/'
    ...
    tic_outer = time.time()
    assignment_files = FilePreparation(sim_time_period, path_external_data,txn_file, network_file, time_table_file, output_file_path, Transfer_Walking_Time)
    #print('Generate files for assignment on ' + DATE)
...
def prepare_input(user_file_path, network_file, time_table_file, afc_file, sjsc_file, Transfer_Walking_Time, date, name):
    # print (user_file_path)
    para_list = pd.read_csv(user_file_path).dropna()
    # print(sjsc_file)
    for ix, para in para_list.iterrows():
        out_put_path = main(para,network_file,time_table_file,afc_file,sjsc_file, Transfer_Walking_Time, date, name)
...
if __name__ == "__main__":
    ...
    global TEST
    TEST = 0 #0 means not run with test
    tic = time.time()
    if TEST == 0:
        ...
        print('process opreation file time', time.time() - tic)
        time_table_file ='External_data/Timetable_Weekday_2017-03-16.csv' # same for all other days
        date = '2017-03-16'
        name = '_syn'
        # ==============
        afc_file ='External_data/AFC_TXN_' +  date +  '.csv'
        sjsc_file ='External_data/SJSC_TXN_' +  date +  '.csv'
        # # ==============
        # afc_file ='External_data/AFC_TXN_2017-03-16.csv'
        # sjsc_file ='External_data/SJSC_TXN_2017-03-16.csv'
        # ==============
        # afc_file ='External_data/AFC_TXN_2017-03-15.csv'
        # sjsc_file ='External_data/SJSC_TXN_2017-03-15.csv'
        # ==============
        # afc_file ='External_data/AFC_TXN_2017-03-14.csv'
        # sjsc_file ='External_data/SJSC_TXN_2017-03-14.csv'
        # ==============
        Transfer_Walking_Time = 'External_data/Transfer_Walking_Time.csv'
        # afc_file ='External_data/AFC_TXN_TEST.csv'
        # sjsc_file ='External_data/SJSC_TXN_TEST.csv'
        out_put_path = prepare_input(user_configuration, network_file, time_table_file, afc_file, sjsc_file, Transfer_Walking_Time, date, name)
    else:
        user_configuration = '0_user_configuration_parameter_TEST.csv'
        mtr_raw_assignment_file_path = 'External_data/MRT_Network_Operation_TEST.csv'
        network_file = post_process(user_configuration, mtr_raw_assignment_file_path, output = True)
        time_table_file ='External_data/Time_table_TEST.csv'
        afc_file ='External_data/AFC_TXN_TEST.csv'
        sjsc_file ='External_data/SJSC_TXN_TEST.csv'
        Transfer_Walking_Time = 'External_data/Transfer_Walking_Time_TEST.csv'
        out_put_path = prepare_input(user_configuration, network_file, time_table_file, afc_file, sjsc_file, Transfer_Walking_Time)
    print('Assignment data preparation finished!\n')