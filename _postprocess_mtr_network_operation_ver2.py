...
def add_new_transfer(para,assignment_file):
    ...
    fraction_Y = int(para['incl_cen_file'].split('CEN')[1])/100
    ...
    def add_daley(x):
        ...
        x.loc[x['DELAY_TIME']>=delay_time,'DELAY_TIME'] += 6.7#6.7 #addtional time
        return x
    ...
    if len(new_path_list)>0:
        ...
        mtr_file.loc[mtr_file_index['index'],['processed']] = 1
        new_path['processed'] = 2
...
def post_process(user_file_path,assignment_file,output):
    # print (user_file_path)
    para_list = pd.read_csv(user_file_path).dropna()
    for ix, para in para_list.iterrows():
        mtr_file = add_new_transfer(para,assignment_file)
    if output:
        mtr_file.to_csv('External_data/mtr_network_operation_assignment_'+para['incl_cen_file']+'.csv', index=False,
                        columns=['ORI_STN_NO', 'DES_STN_NO', 'HARBOUR_CO', 'PASS_LINE_NO','PASS_DIRECTION','PASS_STN','LINK_START',
                                 'LINK_END','DELAY_TIME','MP_SHARE', 'OP_SHARE', 'EP_SHARE','OTHER_SHARE','processed'])
    return mtr_file[['ORI_STN_NO', 'DES_STN_NO', 'HARBOUR_CO', 'PASS_LINE_NO','PASS_DIRECTION','PASS_STN','LINK_START',
                                 'LINK_END','DELAY_TIME','MP_SHARE', 'OP_SHARE', 'EP_SHARE','OTHER_SHARE']]
