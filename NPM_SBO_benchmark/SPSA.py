...
class SimpleSPSA(object):
    ...
    def __init__(self, loss_function, a_par=1e-6, noise_var=0.01, args=(),
                 ...
                 function_tolerance=None, max_iter=5000000,n_jobs=1,random_seed = 0):
    ...
    def minimise(self, theta_0, report=500):
        ...
        :param ens_size: Number of realizations to approximate the gradient.
        ...
        while n_iter < self.max_iter: #(np.linalg.norm(theta_saved - theta) / np.linalg.norm(theta_saved) > 1e-8) and
            ...
            if self.n_jobs == 1:
                ...
                for j in np.arange(ens_size):
                    ...
                    np.random.seed(round(self.random_seed + j + n_iter))
                ...
                ens_size = int(self.n_jobs/2) #n_jobs is an even number
                theta_plus_list = []
                theta_minus_list = []
                for j in np.arange(ens_size):
                    # This loop produces ``ens_size`` realisations of the gradient
                    # which will be averaged. Each has a cost of two function runs.
                    # Bernoulli distribution with p=0.5
                    np.random.seed(round(self.random_seed + j + n_iter + 1))
                    delta = (np.random.randint(0, 2, num_p) * 2 - 1)
                    # Stochastic perturbation, innit
                    theta_plus = theta + ck * delta
                    theta_plus = np.minimum(theta_plus, self.max_vals)
                    theta_minus = theta - ck * delta
                    theta_minus = np.maximum(theta_minus, self.min_vals)
                    theta_plus_list.append(theta_plus)
                    theta_minus_list.append(theta_minus)
                    # Funcion values associated with ``theta_plus`` and
                    # ``theta_minus``
                j_list = list(self.executor.map(self.calc_loss, theta_plus_list + theta_minus_list))
                for j in np.arange(ens_size):
                    j_plus = j_list[j]
                    j_minus = j_list[j+ens_size]
                    ghat = ghat + (j_plus - j_minus) / (2. * ck * delta)
                # Average gradient...
                ghat = ghat / float(ens_size)
